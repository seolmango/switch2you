...
class RigidBody {
  ...
  static isPreCollision(rigidBody1, rigidBody2) {
    return (
      Math.max(
        rigidBody2.pos.x - rigidBody2.shape.checkLeft - (rigidBody1.pos.x + rigidBody1.shape.checkRight),
        rigidBody1.pos.x - rigidBody1.shape.checkLeft - (rigidBody2.pos.x + rigidBody2.shape.checkRight)
      ) >= 0 ||
      Math.max(
        rigidBody2.pos.y - rigidBody2.shape.checkDown - (rigidBody1.pos.y + rigidBody1.shape.checkUp),
        rigidBody1.pos.y - rigidBody1.shape.checkDown - (rigidBody2.pos.y + rigidBody2.shape.checkUp)
      ) >= 0
    );
  }
  ...
  static isCollision(checkType, rigidBody1, rigidBody2) {
    ...
    if (checkType === 'Circle-Circle') {
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        if (axis1.dot(dPos) > 0 && axis2.dot(dPos) > 0) {
          // SAT로 감지 불가능한 circle 좌표 판별
      ...
      if (normal.dot(relativePos) < 0)
        // 기본적으로, 법선벡터와 강체1 기준 좌표차의 내적이 반대방향이니 이 조건은 법선벡터가 강체2 꺼라는 걸 의미함.
  ...
  static CorrectionCollision(rigidBody1, rigidBody2, normal, penetration) {
    ...
    rigidBody1.pos = rigidBody1.pos.minus(
      distance.multiply(rigidBody1.invMass / (rigidBody1.invMass + rigidBody2.invMass))
    ); // 질량을 이용해 조금 더 정확한 보정
    rigidBody2.pos = rigidBody2.pos.plus(
      distance.multiply(rigidBody2.invMass / (rigidBody1.invMass + rigidBody2.invMass))
    );
  }
  ...
  static ResolveCollision(rigidBody1, rigidBody2, normal, contactPoints) {
    ...
    for (let i = 0; i < contactPoints.length; i++) {
      ...
      // mass tanget??
      jt /=
        rigidBody1.invMass + rigidBody2.invMass + f1t * f1t * rigidBody1.invInertia + f2t * f2t * rigidBody2.invInertia;
