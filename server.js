...
import { Vector2 } from './server/Physics/index.js';
...
function getPlayerInfo(players) {
  const infoFilter = (player) => {
    return { number: player.number, name: player.name, role: player.role, device: player.device, skill: player.skill };
  };
  if (Array.isArray(players)) return players.map(infoFilter);
  else return infoFilter(players);
}

function getRoomInfo(rooms) {
  const infoFilter = (room) => {
    return {
      id: room.id,
      name: room.name,
      ownerName: room.owner.name,
      passwordExist: room.password ? true : false,
      playerCount: room.players.length,
      playing: room.playing,
    };
  };
  if (Array.isArray(rooms)) return rooms.map(infoFilter);
  else return infoFilter(rooms);
}
...
function getRigidBodyInfos(rigidBodys) {
  const infoFilter = (rigidBody) => {
    ...
    if (rigidBody.shape.type === 'Circle') info['radius'] = rigidBody.shape.radius;
    else if (rigidBody.shape.type === 'Convex') info['points'] = rigidBody.shape.rotationedPoints;
    return info;
  };
  if (Array.isArray(rigidBodys)) return rigidBodys.map(infoFilter);
  else return infoFilter(rigidBodys);
}
...
function checkData(...args) {
  for (const element of args) {
    ...
    for (let i = 1; i < element.length; i++)
      ...
      } else if (element[0] === element[i]) wrong = false;
...
io.on('connection', (socket) => {
  // Player(세션) 추가
  if (Player.MaxCount <= Player.Count) {
    // 서버가 꽉찼다면
  ...
  socket.on('get room list', (page, callback) => {
    ...
    callback({
      status: 200,
      maxPage: maxPage,
      roomInfos: getRoomInfo(roomList.slice(page * showNum - showNum, maxIndex)),
    });
  });

  // 방 생성
  socket.on('create room', (playerName, roomName, isPublic, password, callback) => {
    // 1. 방 참가 이벤트가 들어오면
    if (
      !checkData(
        [playerName, 'string'],
    ...
    const room = new Room(player, roomName, isPublic, password); // 3. 입력한 (남에게 보여지는) 정보를 업데이트 + 실제 작업 진행
  ...
  socket.on('join room', (playerName, roomId, password, callback) => {
    ...
    callback({
      status: 200,
      roomInfo: getRoomInfo(player.room),
      playerInfos: getPlayerInfo(player.room.players),
  ...
  for (let room in Room.Playings) {
    ...
    for (let player of room.players) {
      ...
      if (actions.move.doing)
        world.rigidBodies[player.number - 1].pos.plus(
          new Vector2(Math.cos(actions.move.direction), Math.cos(actions.move.directoin)).multiply(
